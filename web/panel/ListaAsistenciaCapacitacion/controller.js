...
app.config(function($routeProvider){
	...
	.when('/Departamental/:idEvento/:idDepartamento', {
            templateUrl : "templates/Departamental.jsp",
            controller : "appController"
        })
	...
	.when('/Zonal/:idEvento/:idDepartamento/:idMunicipio/:idCargo', {
            templateUrl : "templates/Zonal.html",
            controller : "appController"
        })
	...
	.when('/seleccionarArchivo/:idDpto/:idMpio/:idZona/:idPuesto/:idCargo/:idEvento', {
...
app.controller("appControllerFiles", ['$scope', '$routeParams', function ($scope, $routeParams){
      ...
      $scope.idPuesto = $routeParams.idPuesto;
...
app.controller("appController", ['$scope', '$routeParams', '$location','SeguimientoFactory', function seguimientoController($scope, $routeParams, $location, SeguimientoFactory){
    ...
    $scope.getEventos = function(){
        ...
        SeguimientoFactory.getEmpleadoPruebaSesion(function (data) {
            ...
            SeguimientoFactory.getEventos($scope.idprueba, function (data) {
                $scope.evento = data;
            });
        });
    };
        ...
        $scope.listar = function(){
            ...
            SeguimientoFactory.listarUbicacion($routeParams.idEvento,$routeParams.idDepartamento,$routeParams.idMunicipio,$routeParams.idCargo,$routeParams.idZona,$routeParams.idPuesto,function (data) {
               ...
               for(m= 0; m < $scope.ubicaciones.length; m++)
                {
                    if ( $scope.ubicaciones[m].estado.codigoEstado=='1' || $scope.ubicaciones[m].estado.codigoEstado=='3')
                    {
                    ...
                    if ( $scope.ubicaciones[m].estado.codigoEstado=='1' || $scope.ubicaciones[m].estado.codigoEstado=='3')
                    {
                        ...
                        if ($scope.ubicaciones[m].cargo.codigoCargo == '1')
                        {
                            $scope.ubicaciones[m].display3 = '';
                        }else
                            {
                                $scope.ubicaciones[m].display3 = 'none';
                            }
                    }else
                        {
                            $scope.ubicaciones[m].display = 'none';
            ...
            SeguimientoFactory.listarSoportes($routeParams.idEvento,$routeParams.idDepartamento,$routeParams.idMunicipio,$routeParams.idCargo,$routeParams.idZona,$routeParams.idPuesto,function (dataSoportes) {
                if(dataSoportes.soporteAsistencia.length > 1)
                {
                    $scope.soportes = dataSoportes.soporteAsistencia;
                }
                else
                    {
                        $scope.soportes = dataSoportes;
                    }
            });
                        ...
                        if (response.codigo == '200')
                            {
                                SeguimientoFactory.getPersonaAsignada(idPersonaAsignada, function (data) {
                                    ...
                                    if($scope.ubicaciones[id].asistio == "true")
                                    {
                                        $scope.ubicaciones[id].asistio = true;
                                    }else
                                    {
                                        $scope.ubicaciones[id].asistio = false;
                                    }
                                    
                                    
                                    if($scope.ubicaciones[id].estado.codigoEstado=='1' || $scope.ubicaciones[id].estado.codigoEstado=='3')
                                    {
                                        $scope.ubicaciones[id].display = "";
                                        $scope.ubicaciones[id].display2 = "none";
                                        
                                        if ($scope.ubicaciones[id].cargo.codigoCargo == '1')
                                        {
                                            $scope.ubicaciones[id].display3 = '';
                                        }else
                                            {
                                                $scope.ubicaciones[id].display3 = 'none';
                                            }
                                    }else
                                        {
