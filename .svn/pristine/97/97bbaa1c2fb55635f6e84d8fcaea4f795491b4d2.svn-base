...
app.config(function($routeProvider){
	...
	.when('/Puesto/:idEvento/:idDepartamento/:idMunicipio/:idCargo/:idZona/:nombreZona', {
            templateUrl : "templates/Puesto.html",
            controller : "appController"
        })
...
app.controller("appControllerUbicacion", ['$scope', '$routeParams', '$location','SeguimientoFactory','PersonaAsignada', function seguimientoController($scope, $routeParams, $location, SeguimientoFactory, PersonaAsignada){
        ...
        $scope.listar = function(){
            ...
            $scope.estados = [
                ...
                {"codigoEstado":"4","descripcion":"APROBADO"}
            ];
            ...
            SeguimientoFactory.listarUbicacion($routeParams.idEvento,$routeParams.idDepartamento,$routeParams.idMunicipio,$routeParams.idCargo,$routeParams.idZona,$routeParams.idPuesto,function (data) {
               ...
               for(m= 0; m < $scope.ubicaciones.length; m++)
                {
                    if($scope.ubicaciones[m].estado.codigoEstado=='2')
                    {
                        ...
                        if (response.codigo == '200')
                            {
                                SeguimientoFactory.getPersonaAsignada(idPersonaAsignada, function (data) {
                                    ...
                                    if($scope.ubicaciones[id].estado.codigoEstado=='2')
                                    {
